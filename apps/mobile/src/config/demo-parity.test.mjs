...
test("adds the Champion calendar and performance management flow", () => {
  ...
  assert.match(performance, /PROFILO PUBBLICO/);
  assert.match(performance, /performance-headline/);
  assert.match(performance, /performance-bio/);
  assert.match(performance, /performance-fan-message/);
  assert.match(performance, /performance-offer-note/);
  assert.match(performance, /SERVIZI E PREZZI/);
  assert.match(performance, /performance-description-/);
  assert.match(performance, /QUANDO SONO DISPONIBILE/);
  assert.match(performance, /availability-service-/);
  assert.match(performance, /LIVE_SLOT_DURATIONS = \[30, 45, 60\]/);
  assert.match(performance, /SESSION_DURATIONS = \[900, 1800, 2700\]/);
  assert.match(performance, /10 secondi tra le chiamate/);
  assert.match(performance, /performance-public-preview/);
  assert.match(performance, /performanceProfiles\.save/);
  assert.match(performance, /7 giorni/);

  assert.match(store, /ChampionPublicProfile/);
  assert.match(store, /publicDescription: string/);
  assert.match(store, /serviceKey: PerformanceServiceKey/);
  assert.match(store, /normalizePerformanceProfile/);
  assert.match(store, /turnaroundSeconds: 10/);
  assert.match(store, /isLive \? profile\.turnaroundSeconds : 0/);
  assert.match(store, /requiredGap = requestedIsLive \|\| existingIsLive \? 10_000 : 0/);
  assert.match(store, /10 secondi tra le chiamate/);
  assert.match(store, /status = "refunded"/);
  assert.match(champion, /performanceProfiles\.get/);
  assert.match(champion, /champion-fan-message/);
  assert.match(champion, /champion-offer-note/);
  assert.match(champion, /preference\?\.publicDescription/);
